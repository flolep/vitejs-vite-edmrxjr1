...
export default function GameEndDashboard({
  ...
  if (leaderboard.length === 0) {
    return (
      <div style={{ height: '100%', overflow: 'hidden', background: COLORS.bg, color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
  ...
  const Bar = ({ value, max, color, label, suffix }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.35rem' }}>
      <div style={{ width: '5rem', fontSize: '0.75rem', opacity: 0.6, textAlign: 'right', flexShrink: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{label}</div>
      <div style={{ flex: 1, background: 'rgba(255,255,255,0.06)', borderRadius: '0.25rem', height: '1.25rem', overflow: 'hidden' }}>
        <div style={{ width: `${Math.max(2, (value / (max || 1)) * 100)}%`, background: color, height: '100%', borderRadius: '0.25rem', transition: 'width 0.8s ease' }} />
      </div>
      <div style={{ width: '3.5rem', fontSize: '0.8rem', fontWeight: 600, fontFamily: 'monospace' }}>{typeof value === 'number' && value % 1 !== 0 ? value.toFixed(1) : value}{suffix || ''}</div>
    </div>
  );

  return (
    <div style={{ height: '100%', overflowY: 'auto', overflowX: 'hidden', background: COLORS.bg, color: 'white', padding: '2rem 3rem', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
        ...
        <div style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: '1rem', padding: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
            {leaderboard.map((player, i) => {
              ...
              return (
                <div key={player.playerName || i} style={{ display: 'flex', alignItems: 'center', gap: '0.85rem', padding: '0.75rem 0.85rem', background: i === 0 ? 'rgba(251,191,36,0.08)' : 'rgba(255,255,255,0.02)', borderRadius: '0.75rem', border: i === 0 ? '1px solid rgba(251,191,36,0.25)' : '1px solid transparent' }}>
                  <span style={{ fontSize: '2rem', minWidth: '2.5rem', textAlign: 'center' }}>{medals[i] || `${i + 1}.`}</span>
                  <div style={{ width: '2.5rem', height: '2.5rem', borderRadius: '50%', background: color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1rem', fontWeight: 700, flexShrink: 0 }}>{getInitials(player.playerName)}</div>
                  ...
                  <div style={{ flex: 1, padding: '0 1rem' }}>
                    <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: '0.35rem', height: '1rem', overflow: 'hidden' }}>
                      <div style={{ width: `${(player.totalPoints / maxScore) * 100}%`, background: color, height: '100%', borderRadius: '0.35rem', transition: 'width 1s ease' }} />
                    </div>
                  </div>
                  <div style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: '1.35rem', color: i === 0 ? COLORS.accent : 'white', minWidth: '5rem', textAlign: 'right' }}>{player.totalPoints} pts</div>
                </div>
              );
            })}
          </div>
